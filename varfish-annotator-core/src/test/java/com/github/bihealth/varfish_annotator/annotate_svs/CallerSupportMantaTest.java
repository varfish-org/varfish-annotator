...
public class CallerSupportMantaTest {

  @TempDir public File tmpFolder;
  File vcfFile;
  File otherVcfFile;
  CallerSupportManta callerSupport;

  @BeforeEach
  void initEach() {
    vcfFile = new File(tmpFolder + "/vcf-header.vcf");
    ResourceUtils.copyResourceToFile("/callers-sv/manta-head.vcf", vcfFile);
    otherVcfFile = new File(tmpFolder + "/incompatible.vcf");
    ResourceUtils.copyResourceToFile("/callers-sv/delly2-head.vcf", otherVcfFile);
    callerSupport = new CallerSupportManta();
  }

  @Test
  void testGetSvCaller() {
    Assertions.assertEquals(SvCaller.MANTA, callerSupport.getSvCaller());
  }
  ...
  void testIsCompatiblePositive() {
    ...
    Assertions.assertEquals(callerSupport.getVersion(vcfReader), "1.6.0");
  }
